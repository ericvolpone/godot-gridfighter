...
@onready var koth_manager: KothManager = get_parent();
var score_label_by_player: Dictionary = {}

func _ready() -> void:
	for player_name: String in koth_manager.score_by_player:
		add_player_to_score(player_name)

# Called every frame. 'delta' is the elapsed time since the previous frame.
func _physics_process(delta: float) -> void:
	for player_name: String in koth_manager.score_by_player:
		var score: int = koth_manager.score_by_player[player_name]
